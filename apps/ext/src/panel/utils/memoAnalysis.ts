...
export function analyzeMemoization(commits: CommitData[]): MemoReport[] {
    ...
    for (const fiber of memoFibers) {
      ...
      if (record.propsHistory.length > 0) {
        ...
        if (shallowEqual(prevProps, currentProps)) {
          ...
          // (This indicates the memo was bypassed or outer props are not memoized)
          record.memoMisses += 1;
        } else {
  ...
  for (const record of records.values()) {
    // A memo "hit" is when the component did NOT render despite parent re-rendering.
    // Since we only see fibers that DID render, hits = total renders where props were unchanged
    // compared to the previous render — but the component still rendered, so memo didn't prevent it.
    //
    // Effective hit rate: out of all renders after the first, how many had the same props as previous?
    // If many are the same, memo is ineffective (it should have prevented the render).
    ...
    // Hit rate: (renders where memo should have prevented) / total renders after first
    // A high samePropRenders rate means memo is NOT effective
    const ineffectiveRate = rendersAfterFirst > 0 ? samePropRenders / rendersAfterFirst : 0;
    const currentHitRate = Math.round((1 - ineffectiveRate) * 1000) / 10; // percentage of useful renders
