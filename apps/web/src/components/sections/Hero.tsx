...
const EXTENSION_DOCS_URL = 'https://github.com/rejisterjack/react-perf-profiler#install';

type Browser = 'chrome' | 'firefox';

/**
 * Try to download a prebuilt browser ZIP. If the file does not exist (404),
 * fall back to the documented "load unpacked" path so the CTA never breaks.
 *
 * The fallback runs on the client because the build may produce the ZIPs at
 * release time without the dev server knowing.
 */
function useBrowserInstaller() {
  const [fallback, setFallback] = useState<Browser | null>(null);

  async function install(browser: Browser) {
    const zipUrl = `${DOWNLOAD_BASE}/react-perf-profiler-${browser}.zip`;

    try {
      const res = await fetch(zipUrl, { method: 'HEAD' });
      if (res.ok) {
        // ZIP exists — trigger a real download.
      ...
      // Network/CORS error on HEAD — fall through to docs.
    }
...
function BuildFromSourceNote({ browser }: { browser: Browser }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      className="mt-4 rounded-lg border border-brand-amber/30 bg-brand-amber/5 px-4 py-3 text-sm text-surface-300"
    >
      <p className="font-semibold text-brand-amber mb-1">
        Pre-built {browser === 'chrome' ? 'Chrome' : 'Firefox'} ZIP not available yet.
      </p>
      <p className="mb-2">Build from source in two commands (MIT licensed, no signup required):</p>
      <pre className="overflow-x-auto rounded bg-surface-900/70 px-3 py-2 text-xs text-surface-200">
        <code>{`git clone https://github.com/rejisterjack/react-perf-profiler.git
cd react-perf-profiler/apps/ext && bun install && bun run build`}</code>
      </pre>
      <p className="mt-2">
        Then load `apps/ext/.output/{browser}-{browser === 'chrome' ? 'mv3' : 'mv2'}/` as an
        unpacked extension.{' '}
        <a
          href={EXTENSION_DOCS_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="text-brand-blue hover:text-brand-cyan underline underline-offset-2"
        >
          Full install guide ↗
        </a>
      </p>
...
export const Hero = () => {
  const { install, fallback } = useBrowserInstaller();
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 section-padding">
      ...
      >
        ...
        <motion.div
          ...
          className="mb-6 inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5 backdrop-blur-sm text-sm text-brand-cyan"
        >
        ...
        >
          The most advanced DevTools extension for profiling React applications. Identify wasted
          renders, score memoization, and optimize RSC payloads in seconds — without switching
          tools.
        </motion.p>
        ...
        {/* Per-browser fallback if ZIP not yet built */}
        {fallback && <BuildFromSourceNote browser={fallback} />}

        {/* Trust micro-copy */}
        <motion.div
          variants={fadeUpStagger}
          className="flex flex-wrap items-center justify-center gap-6 text-sm text-surface-500 mb-4"
        >
        ...
        <motion.p variants={fadeUpStagger} className="text-xs text-surface-600 mb-16">
          ...
          <a
            href="#demo"
            className="text-brand-blue hover:text-brand-cyan transition-colors underline underline-offset-2"
          >
        ...
        {/* Mockup */}
        <motion.div variants={fadeUp} className="relative mx-auto max-w-5xl perspective-[1000px]">
            ...
            <div className="w-full aspect-[16/9] bg-gradient-to-br from-surface-800 via-surface-850 to-surface-900 relative flex flex-col overflow-hidden">
              ...
              <div className="flex items-center gap-2 px-4 py-2.5 bg-surface-900/80 border-b border-white/5 shrink-0">
                ...
                <div className="flex items-center gap-3 ml-4 text-xs text-surface-500 font-mono">
                  <span className="px-2 py-0.5 rounded bg-surface-700/50 text-surface-400">
                    Elements
                  </span>
                  <span className="px-2 py-0.5 rounded bg-surface-700/50 text-surface-400">
                    Console
                  </span>
                  <span className="px-2 py-0.5 rounded bg-brand-blue/20 text-brand-cyan border border-brand-blue/30">
                    ⚡ Perf Profiler
                  </span>
                  <span className="px-2 py-0.5 rounded bg-surface-700/50 text-surface-400">
                    Network
              ...
              <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-56 border-r border-white/5 p-3 space-y-1 shrink-0">
                  <div className="text-[10px] font-bold text-surface-500 uppercase tracking-widest mb-2 px-1">
                    Component Tree
                  </div>
                  {[
                    ...
                    >
                      <span className={c.wasted > 0 ? 'text-red-300' : 'text-surface-400'}>
                        {c.name}
                      </span>
                      {c.wasted > 0 && <span className="text-red-400 font-mono">{c.wasted}w</span>}
                    </div>
                  ))}
                </div>
                {/* Main panel - flamegraph */}
                <div className="flex-1 p-4">
                  <div className="text-[10px] font-bold text-surface-500 uppercase tracking-widest mb-3">
                    Flamegraph — Commit #7
                  </div>
                  <div className="space-y-1.5">
                    <div
                      className="h-6 rounded bg-brand-cyan/20 border border-brand-cyan/30 flex items-center px-2 text-[10px] text-brand-cyan"
                      style={{ width: '100%' }}
                    >
                      App — 45ms total
                    </div>
                    <div
                      className="h-5 rounded bg-brand-blue/20 border border-brand-blue/30 flex items-center px-2 text-[10px] text-brand-blue ml-4"
                      style={{ width: '85%' }}
                    >
                      Feed — 32ms
                    </div>
                    <div
                      className="h-5 rounded bg-red-500/30 border border-red-500/40 flex items-center px-2 text-[10px] text-red-300 ml-8"
                      style={{ width: '72%' }}
                    >
                      Post — 28ms ⚠ wasted
                    </div>
                    <div
                      className="h-5 rounded bg-red-500/50 border border-red-500/60 flex items-center px-2 text-[10px] text-red-200 ml-12"
                      style={{ width: '55%' }}
                    >
                      Actions — 18ms ⚠ 100% wasted
                    </div>
                    <div
                      className="h-4 rounded bg-surface-700/40 flex items-center px-2 text-[10px] text-surface-500 ml-4"
                      style={{ width: '30%' }}
                    >
                      Header — 2ms ✓
                    </div>
                  ...
                  <div className="mt-4 rounded-lg bg-brand-cyan/5 border border-brand-cyan/20 p-3">
                    <div className="text-[10px] font-bold text-brand-cyan mb-1">
                      ⚡ Fix Recommendation — Actions.tsx
                    </div>
                    <div className="text-[10px] text-surface-400">
                      <span className="text-red-400">onLike</span> callback recreated on every
                      render. <span className="text-brand-green">Wrap with useCallback</span> →
                      expected 94% wasted render reduction.
                    </div>
                  </div>
                </div>
              </div>
