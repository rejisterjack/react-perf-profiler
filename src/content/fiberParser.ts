 ...
 */
function getPriorityLevelName(level: number): CommitData['priorityLevel'] {
  switch (level) {
    case 1: return 'Immediate';
    case 2: return 'UserBlocking';
...
export function getComponentName(fiber: unknown): string {
  ...
  const fiberObj = fiber as Record<string, unknown>;
  const { type, tag, elementType } = fiberObj;
    ...
    case 11: // ForwardRef
      if (typeof type === 'object' && type !== null) {
        ...
        if (typeof renderFn === 'function') {
          return (renderFn as { displayName?: string; name?: string }).displayName || 
    ...
    case 15: // SimpleMemoComponent
      if (typeof type === 'object' && type !== null) {
        ...
        if (typeof innerType === 'function') {
          const name = (innerType as { displayName?: string; name?: string }).displayName || 
 ...
 */
export function extractProps(memoizedProps: Record<string, unknown> | undefined): Record<string, unknown> {
    ...
    for (const key of Object.keys(memoizedProps)) {
      ...
      if (value === null || value === undefined) {
        ...
        if (seen.has(value)) {
          ...
          for (const k of Object.keys(value).slice(0, 10)) { // Limit to 10 keys
