...
const FilterIcon: React.FC<{ size?: number }> = ({ size = 14 }) => (
  <svg
    ...
    aria-hidden="true"
  >
    <path d="M1 3H13M3 7H11M5 11H9" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
  </svg>
);
...
const SeverityButton: React.FC<SeverityButtonProps> = memo(
  ({ severity, label, isActive, onToggle, disabled }) => {
...
export const TreeSearch: React.FC<TreeSearchProps> = memo(
  ({
    ...
    const containerClasses = [styles["searchContainer"], className].filter(Boolean).join(' ');
    ...
    return (
      <div className={containerClasses}>
        ...
        <div className={styles["searchSection"]}>
          ...
          {/* Results count */}
          {showResultsCount && (
            <span id="search-results-count" className={styles["resultsCount"]} aria-live="polite">
